...
import {loadPinnedPolicy,assertPolicy,sha256,canonical,validateLights} from '../assets/guards.js';
...
async function boot(){
 ...
 const audit=JSON.parse(new TextDecoder().decode(auditBytes));
 ...
 function fit(){const center=boxes[active].getCenter(new THREE.Vector3());const dir=(preset==='plan'?new THREE.Vector3(0,1,-.001):preset==='front'?new THREE.Vector3(0,.02,-1):new THREE.Vector3(14,11,-19)).normalize();const right=new THREE.Vector3().crossVectors(camera.up,dir).normalize(),up=new THREE.Vector3().crossVectors(dir,right);const tv=Math.tan(THREE.MathUtils.degToRad(camera.fov/2)),th=tv*camera.aspect;let dist=1;for(const p of corners()){const d=p.sub(center),v=d.dot(dir);dist=Math.max(dist,Math.abs(d.dot(right))/(th*.80)+v,Math.abs(d.dot(up))/(tv*.70)+v);}camera.position.copy(center).addScaledVector(dir,dist*1.05);controls.target.copy(center);controls.update();dirty=true;}
 ...
 function renderLabels(){for(const l of labels){const p=l.point.clone().project(camera),visible=l.root===active&&Math.abs(p.x)<1.05&&Math.abs(p.y)<1.05&&Math.abs(p.z)<1;l.el.hidden=!visible;if(visible){l.el.style.left=(p.x*.5+.5)*container.clientWidth+'px';l.el.style.top=(-p.y*.5+.5)*container.clientHeight+'px';}}}
