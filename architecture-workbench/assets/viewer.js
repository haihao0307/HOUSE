...
export async function createViewer(container,policy,build){
 ...
 function resize(){const w=Math.max(1,container.clientWidth),h=Math.max(1,container.clientHeight);renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();dirty=true;document.querySelector('#pixelSize').textContent=`原生 ${renderer.domElement.width} × ${renderer.domElement.height} px`;}
 function render(){renderer.render(scene,camera);frames++;dirty=false;}
 function setView(id){const v=PRESETS[id];if(!v)throw new Error('未知视角');camera.position.fromArray(v.p);controls.target.fromArray(v.t);camera.updateProjectionMatrix();controls.update();dirty=true;}
