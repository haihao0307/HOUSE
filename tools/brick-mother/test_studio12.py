...
def ready(page):
 ...
 check('runtime healthy',page.evaluate("document.documentElement.dataset.runtimeFailure!=='true'"),page.locator('#error').inner_text())
...
def screenshot(page,name):
 ...
 check('screenshot '+name,(OUT/(name+'.png')).stat().st_size>15000)
with sync_playwright() as pw:
 ...
 try:
  ...
  ready(page);report['initialReadyMs']=round((time.monotonic()-start)*1000)
  ...
  check('actual version',page.evaluate(API+'.version')=='1.2.0-alpha.9')
  check('six families',page.locator('.family-btn').count()==6)
  ...
  for family in range(6):
   wake(page);page.click('#showMaterials');page.click(f'.family-btn[data-family="{family}"]');ready(page)
   check('material '+str(family),page.evaluate(API+'.state.family')==family)
   wake(page);page.click('#hidePanels');page.wait_for_timeout(450);screenshot(page,'family-'+str(family))
  wake(page);page.click('#showControls')
  for channel in range(16):
   page.click(f'.diag[data-view="{channel}"]');page.wait_for_timeout(110)
   check('diagnostic '+str(channel),page.evaluate(API+'.state.viewMode')==channel)
  page.click('.diag[data-view="0"]')
  for light in ['studio','neutral','raking','overcast','outdoor']:
   page.click(f'.light-btn[data-light="{light}"]');page.wait_for_timeout(250)
   check('light '+light,page.evaluate(API+'.state.lightMode')==light)
  ...
  page.click('#playPause');page.wait_for_timeout(400);t0=page.evaluate(API+'.state.simTime');page.wait_for_timeout(700);t1=page.evaluate(API+'.state.simTime')
  check('evolution advances',t1>t0)
  page.click('#playPause');page.wait_for_timeout(250);t0=page.evaluate(API+'.state.simTime');page.wait_for_timeout(300)
  check('pause preserves time',page.evaluate(API+'.state.simTime')==t0)
  page.click('#rainPulse');page.wait_for_timeout(400);check('rain resumes',page.evaluate(API+'.state.playing') is True);page.click('#playPause')
  wake(page);page.click('#hidePanels');page.wait_for_timeout(600)
  old=page.evaluate(API+'.camera.goalYaw');page.mouse.move(540,410);page.mouse.down();page.mouse.move(680,445,steps=12);page.mouse.up();page.wait_for_timeout(1000)
  check('orbit drag',abs(page.evaluate(API+'.camera.goalYaw')-old)>.1)
  ...
  page.click('#fullScreen');page.wait_for_timeout(400);check('fullscreen gesture',page.evaluate('!!document.fullscreenElement'));page.evaluate('document.exitFullscreen()')
  page.wait_for_timeout(1800);n0=page.evaluate('Number(document.documentElement.dataset.renderCount)');page.wait_for_timeout(650);n1=page.evaluate('Number(document.documentElement.dataset.renderCount)')
  check('idle rendering stops',n1-n0<=2,{'before':n0,'after':n1})
  page.wait_for_timeout(5500);check('automatic immersive mode',page.evaluate("document.body.classList.contains('immersive')"))
  ...
  mobile.click('#showControls');check('mobile parameter drawer',mobile.evaluate("document.body.classList.contains('right-open')"));screenshot(mobile,'mobile-controls')
  mobile.keyboard.press('Escape');screenshot(mobile,'mobile-immersive')
  ...
  report['screenshots']=[{'name':p.name,'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in sorted(OUT.glob('*.png'))]
  (OUT/'verification.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
  browser.close()
