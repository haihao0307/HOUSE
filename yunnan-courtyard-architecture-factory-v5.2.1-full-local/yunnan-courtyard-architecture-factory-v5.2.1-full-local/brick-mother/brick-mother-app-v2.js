...
const state = {
  ...
  // Zero is the canonical fired-clay slot; do not let `|| 1` silently
  // redirect focus=0 to the stone child in evidence captures.
  evidenceFocus: (() => {
    const requested = Number(QUERY.get('focus'));
    return Number.isFinite(requested) ? Math.max(0, Math.min(2, Math.round(requested))) : 1;
  })(),
