...
function faceRelief(family,kind,p,seed,controls,scale,faceRand){
  ...
  else if(family===1){amp=kind===1?.014:kind>=3?.036:.028;detail=macro*.58+meso*.25+ridge*.17;}
...
function polygonAreaXZ(poly){let a=0;for(let i=0;i<poly.length;i++){const p=poly[i],q=poly[(i+1)%poly.length];a+=p.x*q.z-q.x*p.z;}return a*.5;}
function ensureCCWXZ(poly){return polygonAreaXZ(poly)<0?poly.slice().reverse():poly;}
function scaleFootprint(poly,scaleFactor,shift=v3()){return poly.map(p=>v3(p.x*scaleFactor+shift.x,0,p.z*scaleFactor+shift.z));}
function chamferedRectangle(hx,hz,ch){return ensureCCWXZ([
  v3(-hx+ch,0,-hz),v3(0,0,-hz),v3(hx-ch,0,-hz),v3(hx,0,-hz+ch),v3(hx,0,0),v3(hx,0,hz-ch),
  v3(hx-ch,0,hz),v3(0,0,hz),v3(-hx+ch,0,hz),v3(-hx,0,hz-ch),v3(-hx,0,0),v3(-hx,0,-hz+ch)
]);}
function ringPoint(poly,index,y){const p=poly[index];return v3(p.x,y,p.z);}
function appendRingSides(builder,rings,family,seed,controls,quality,scale,breakEdges=new Set(),chipEdges=new Set()){
  const count=rings[0].length,segV=Math.max(2,Math.round(mix(2,5,quality)));
  for(let band=0;band<rings.length-1;band++)for(let edge=0;edge<count;edge++){
    const next=(edge+1)%count,p00=rings[band][edge],p10=rings[band][next],p01=rings[band+1][edge],p11=rings[band+1][next],dx=p10.x-p00.x,dz=p10.z-p00.z,out=norm(v3(dz,0,-dx));
    const edgeLength=Math.max(len(sub(p10,p00)),len(sub(p11,p01))),segU=Math.max(2,Math.round(edgeLength/(.16*scale)*mix(.58,1.05,quality))),isBreak=breakEdges.has(edge),isChip=chipEdges.has(edge),kind=isBreak?4:(isChip?3:2),rows=[];
    const baseAt=(u,v)=>{const a=lerp3(p00,p10,u),b=lerp3(p01,p11,u);return lerp3(a,b,v);};
    const posAt=(u,v)=>{const base=baseAt(u,v),edgeFade=smoothstep(0,.13,Math.min(u,1-u))*smoothstep(0,.16,Math.min(v,1-v)),rel=faceRelief(family,kind,base,seed+edge*149+band*911,controls,scale,(edge+.37)/(count+1)),event=isBreak?(.62+.38*Math.sin((u*.78+v*.22)*Math.PI)):(isChip?.58:clamp(.5+.5*fbm3(base.x*1.1/scale,base.y*1.2/scale,base.z*1.1/scale,seed+edge*37,3),0,1)),offset=rel*(.24+.76*edgeFade)*(isBreak?1.26:1);return{p:add(base,mul(out,offset)),event};};
    for(let iu=0;iu<=segU;iu++){rows[iu]=[];for(let iv=0;iv<=segV;iv++){
      const u=iu/segU,v=iv/segV,q=posAt(u,v),e=.0035,qu0=posAt(clamp(u-e,0,1),v),qu1=posAt(clamp(u+e,0,1),v),qv0=posAt(u,clamp(v-e,0,1)),qv1=posAt(u,clamp(v+e,0,1)),du=sub(qu1.p,qu0.p),dv=sub(qv1.p,qv0.p);let nn=norm(cross(dv,du));if(dot(nn,out)<0)nn=mul(nn,-1);const edgeSignal=Math.max(1-smoothstep(0,.12,Math.min(u,1-u)),1-smoothstep(0,.15,Math.min(v,1-v)));rows[iu][iv]=builder.addVertex(q.p,nn,[kind,(edge+.5)/count,edgeSignal,q.event]);
    }}
    for(let iu=0;iu<segU;iu++)for(let iv=0;iv<segV;iv++){const a=rows[iu][iv],b=rows[iu+1][iv],c=rows[iu][iv+1],d=rows[iu+1][iv+1];builder.tri(a,c,b);builder.tri(b,c,d);}
  }
}
function buildRingStone({family,seed,controls,quality,scale,rings,topPoly,bottomPoly,topHeight,bottomHeight,breakEdges,chipEdges,bounds,grammar,meta={}}){
  const builder=new MeshBuilder();appendFlagFace(builder,topPoly.map(p=>({x:p.x,z:p.z})),topHeight,seed,controls,quality,scale,1);appendFlagFace(builder,bottomPoly.map(p=>({x:p.x,z:p.z})),bottomHeight,seed+17,controls,quality,scale,0);appendRingSides(builder,rings,family,seed,controls,quality,scale,breakEdges,chipEdges);const data=builder.finish({seed,family,controls,bounds,meta:{grammar,...meta}});shiftBottom(data.positions);return data;
}

function createAshlarMesh(seed,controls,quality=1,scale=1){
  const R=new RNG(seed^0x61c88647),hx=mix(1.42,1.55,controls.form)*scale,hz=mix(.86,.99,controls.form)*scale,height=mix(.72,.88,controls.form)*scale,ch=mix(.105,.165,controls.edge)*scale,base=chamferedRectangle(hx,hz,ch),count=base.length;
  const jittered=base.map((p,i)=>{const sideMid=i%3===1,rad=norm(v3(p.x,0,p.z)),tangent=v3(-rad.z,0,rad.x),normalJ=R.range(-.010,.012)*scale*controls.form,tanJ=R.range(-.008,.008)*scale*controls.form*(sideMid?.45:1);return add(p,add(mul(rad,normalJ),mul(tangent,tanJ)));});
  const drift=v3(R.range(-.012,.012)*scale,0,R.range(-.010,.010)*scale),bottom=scaleFootprint(jittered,.975),lower=scaleFootprint(jittered,1.0),upper=scaleFootprint(jittered,.997,drift),top=scaleFootprint(jittered,.977,drift),cornerIndices=[0,2,3,5,6,8,9,11],chipEdges=new Set(),breakEdges=new Set(),chipCount=1+Math.round(controls.fracture*1.8);
  for(let k=0;k<chipCount;k++){
    const idx=cornerIndices.splice(R.int(0,cornerIndices.length-1),1)[0],topChip=R.next()<.76,depth=R.range(.030,.072)*scale*(.55+.65*controls.fracture),affected=[idx,(idx-1+count)%count,(idx+1)%count];
    for(const j of affected){const w=j===idx?1:.34,dir=norm(v3(top[j].x,0,top[j].z));if(topChip){top[j]=sub(top[j],mul(dir,depth*w));upper[j]=sub(upper[j],mul(dir,depth*w*.38));}else{bottom[j]=sub(bottom[j],mul(dir,depth*w));lower[j]=sub(lower[j],mul(dir,depth*w*.34));}chipEdges.add(j);chipEdges.add((j-1+count)%count);}
  }
  if(controls.fracture>.58){const e=R.int(0,count-1);breakEdges.add(e);upper[e]=sub(upper[e],mul(norm(v3(upper[e].x,0,upper[e].z)),.025*scale*controls.fracture));}
  const tiltX=R.range(-.014,.014)*controls.form,tiltZ=R.range(-.016,.016)*controls.form,topHeight=(x,z)=>height+tiltX*x+tiltZ*z+fbm3(x*.52/scale,z*.52/scale,1.7,seed+409,3)*.009*scale*controls.relief,bottomHeight=()=>0;
  const ys=[0,.055*scale,height-.060*scale,height],polys=[bottom,lower,upper,top],rings=polys.map((poly,r)=>poly.map(p=>ringPoint(poly,poly.indexOf(p),r===3?topHeight(p.x,p.z):ys[r])));
  return buildRingStone({family:0,seed,controls,quality,scale,rings,topPoly:top,bottomPoly:bottom,topHeight,bottomHeight,breakEdges,chipEdges,bounds:v3(hx+.18*scale,height+.15*scale,hz+.18*scale),grammar:'ashlar-v35',meta:{chipCount,bevel:true}});
}

function createRubbleMesh(seed,controls,quality=1,scale=1){
  const R=new RNG(seed^0x9e3779b9),h=mix(.58,.79,controls.form)*scale,hx=mix(1.42,1.76,controls.form)*scale,hz=mix(.92,1.24,controls.form)*scale,c=v3(R.range(-.13,.13)*scale,0,R.range(-.11,.11)*scale),planes=[];
  planes.push(plane(v3(0,-1,0),0,0,R.next()));
  const topTilt=norm(v3(R.range(-.085,.085)*controls.form,1,R.range(-.095,.095)*controls.form)),topP=v3(c.x+R.range(-.055,.055)*scale,h,c.z+R.range(-.050,.050)*scale);planes.push(plane(topTilt,dot(topTilt,topP),1,R.next()));
  const crownDir=R.range(0,Math.PI*2);
  const topCutCount=1+Math.round(Math.max(0,controls.fracture-.58)),topOffset=crownDir+R.range(-.28,.28);
  for(let i=0;i<topCutCount;i++){
    const a=topOffset+i*Math.PI*2/Math.max(topCutCount,2)+R.range(-.18,.18),slope=R.range(.22,.38)*(.52+.48*controls.form),n=norm(v3(Math.cos(a)*slope,1,Math.sin(a)*slope)),p=v3(c.x+Math.cos(a)*hx*R.range(.68,.84),h*R.range(.86,.95),c.z+Math.sin(a)*hz*R.range(.68,.84));
    planes.push(plane(n,dot(n,p),4,R.next()));
  }
  const sideCount=7+Math.round(controls.form*2),offset=R.range(0,Math.PI*2),dominant=R.int(0,sideCount-1);
  for(let i=0;i<sideCount;i++){
    const a=offset+i*Math.PI*2/sideCount+R.range(-.12,.12),support=1/Math.sqrt(Math.cos(a)**2/(hx*hx)+Math.sin(a)**2/(hz*hz)),rad=support*R.range(.68,1.08),ny=R.range(-.34,.32)*controls.form+(i===dominant?R.range(.08,.18):0),n=norm(v3(Math.cos(a),ny,Math.sin(a))),p=v3(c.x+Math.cos(a)*rad,h*R.range(.30,.74),c.z+Math.sin(a)*rad);
    planes.push(plane(n,dot(n,p),2,R.next()));
  }
  const cutCount=3+Math.round(controls.fracture*2.6);
  for(let i=0;i<cutCount;i++){
    const a=offset+R.range(0,Math.PI*2),support=1/Math.sqrt(Math.cos(a)**2/(hx*hx)+Math.sin(a)**2/(hz*hz)),up=i===0?R.range(.14,.36):R.range(-.28,.48),n=norm(v3(Math.cos(a)*R.range(.76,1.18),up,Math.sin(a)*R.range(.76,1.18))),p=v3(c.x+Math.cos(a)*support*R.range(.52,.84),h*R.range(.18,.86),c.z+Math.sin(a)*support*R.range(.52,.84));
    planes.push(plane(n,dot(n,p)-R.range(.018,.062)*scale,i===0?4:3,R.next()));
  }
  if(controls.fracture>.56){const a=offset+R.range(0,Math.PI*2),n=norm(v3(Math.cos(a)*.70,R.range(.44,.70),Math.sin(a)*.70)),p=v3(c.x+Math.cos(a)*hx*.31,h*R.range(.66,.88),c.z+Math.sin(a)*hz*.31);planes.push(plane(n,dot(n,p),4,R.next()));}
  return buildConvexStone(planes,1,seed,controls,quality,scale,{bounds:v3(hx+.25*scale,h+.20*scale,hz+.25*scale),grammar:'rubble-v35-convex',stableBase:true,primaryFaceFamily:true});
}
...
function createFlagstoneMesh(seed,controls,quality=1,scale=1){
  ...
  const layers=[];for(let i=0;i<1+Math.round(controls.fracture*1.6);i++)layers.push({edge:R.range(0,count),span:R.range(.32,.82),y:R.range(.25,.74),width:R.range(.032,.065),amp:R.range(.018,.052)*scale*(.55+.80*controls.fracture),sign:R.next()<.30?1:-1});if(controls.fracture>.30&&!layers.some(l=>l.sign>0))layers[0].sign=1;
  ...
  for(let ei=0;ei<count;ei++){
    ...
    for(let iu=0;iu<=segU;iu++){rows[iu]=[];for(let iv=0;iv<=segV;iv++){
      const u=iu/segU,v=iv/segV,q=posAt(u,v),e=.004,qu0=posAt(clamp(u-e,0,1),v),qu1=posAt(clamp(u+e,0,1),v),qv0=posAt(u,clamp(v-e,0,1)),qv1=posAt(u,clamp(v+e,0,1)),n=norm(cross(sub(qv1.p,qv0.p),sub(qu1.p,qu0.p)));if(dot(n,out)<0)n=mul(n,-1);const edge=Math.max(1-smoothstep(0,.10,v),smoothstep(.90,1,v));rows[iu][iv]=builder.addVertex(q.p,n,[2,(ei+.5)/count,edge,q.event]);
    }}
...
function createCobbleMesh(seed,controls,quality=1,scale=1){
  ...
  for(let i=0;i<3;i++){lobeDirs.push(norm(v3(R.range(-1,1),R.range(-.75,.85),R.range(-1,1))));lobeAmp.push(R.range(-.075,.115));}
  for(let i=0;i<2+Math.round(controls.fracture*1.8);i++)impactDirs.push({d:norm(v3(R.range(-1,1),R.range(-.48,.42),R.range(-1,1))),depth:R.range(.022,.060)*scale*(.55+controls.fracture),width:R.range(.070,.145)});
  ...
  for(let i=0;i<base.verts.length;i++){
    const d=base.verts[i],macro=fbm3(d.x*.90,d.y*.90,d.z*.90,seed+701,4),meso=fbm3(d.x*2.6,d.y*2.6,d.z*2.6,seed+743,3);let lobe=0;for(let k=0;k<lobeDirs.length;k++)lobe+=lobeAmp[k]*Math.pow(Math.max(0,dot(d,lobeDirs[k])),2.2);const rad=1+lobe+macro*.060*controls.form+meso*.010*controls.relief;
    ...
    for(const imp of impactDirs){const a=1-dot(d,imp.d),g=Math.exp(-(a*a)/(2*imp.width*imp.width)),rw=imp.width*.62,rim=Math.exp(-((a-imp.width*1.72)*(a-imp.width*1.72))/(2*rw*rw)),ratio=imp.depth/Math.max(rx,ry,rz);p=mul(p,1-ratio*g+ratio*.16*rim);impact=Math.max(impact,g);}
...
if(typeof window!=='undefined')window.BrickMotherStoneFormGeometryV35=api;
})();
