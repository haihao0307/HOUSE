/* Brick Mother R6. Independent procedural specimen kernel, 2026-09-06.
...
function build(c){
 ...
 const hard=isStone&&[1,3,4].includes(c.rock),bedded=isStone&&![1,3,4].includes(c.rock)&&(family==='stone'||[5,6].includes(c.rock)),chisel=c.chisel??.45,strata=c.strata??(family==='stone'?.8:.2),roundness=c.roundness??.85;
 ...
 const dx=2*(Math.max(...h)+.16)/(c.resolution-1),padding=isStone?.28:.18,dims=h.map(a=>Math.ceil(2*(a+padding)/dx)+1),[nx,ny,nz]=dims,N=nx*ny*nz,org=dims.map(n=>-(n-1)*dx/2),stride=nx*ny;
 ...
 const phase=R()*20,radius=soft?(family==='adobe'?.048+wear*.067:.042+wear*.098):.036;
 const beds=[];let level=-h[1]*1.1;while(level<h[1]*1.2){level+=.09+R()*.22;beds.push([level,(R()-.5)*(.06+strata*.23)]);}
 for(let z=0;z<nz;z++)for(let y=0;y<ny;y++)for(let x=0;x<nx;x++){
  ...
  jitter[i]=b;
  let bedOffset=0;if(bedded){const w=py+px*.14+pz*.10;for(const b of beds){if(w>b[0])bedOffset=b[1];else break;}}
  const rLocal=family==='adobe'?radius*(.76+.34*(a+.5)+.28*(b+.5)):radius;
  ...
  let sd=Math.hypot(Math.max(qx,0),Math.max(qy,0),Math.max(qz,0))+Math.min(Math.max(qx,qy,qz),0)-rLocal;
  if(isPebble){const ex=px+.04*a,ey=py+.05*a,ez=pz;const k0=Math.hypot(ex/h[0],ey/h[1],ez/h[2]),k1=Math.hypot(ex/(h[0]*h[0]),ey/(h[1]*h[1]),ez/(h[2]*h[2]));sd=k1>1e-9?k0*(k0-1)/k1:-Math.min(...h);sd+=(1-roundness)*(.055*a+.018*b+.016*Math.sin(px*3+py*2));}
  ...
  if(soft){
   ...
   sd+=(a*(family==='adobe'?.063:.032)+b*.028*(.16+.84*faceWeight))*c.relief;
  }else sd+=a*(family==='rubble'?.16:.060)*c.relief+(b*(isStone?.072:.052)+Math.pow(clamp((b+.5-.35)/.36),.32)*.035)*c.relief*(hard?.26:1);
 ...
 const count=Math.round(c.damage*(isPebble?7:isStone?(hard?22:48):isBrick?66:54));
 for(let k=0;k<count;k++){
  ...
  if(isStone){ru*=hard?.75:1.1;rv*=hard?.72:.58;rd*=hard?.28:.8;}
 ...
 if(c.damage>(soft?.50:0)&&!isPebble&&!hard)for(let k=0;k<(isStone?9:3);k++){
 ...
 if(isStone){
  for(let k=0;k<Math.round(chisel*32);k++){
   ...
   faceEvent(ax,sign,u,v,.055+D()*.11,.018+D()*.018,.018+chisel*.044+(hard?.006:c.damage*.03),angle,'chip');
  }
 }
 if(bedded&&strata>0){for(let k=0;k<beds.length;k++){for(const sign of [-1,1])faceEvent(2,sign,(D()-.5)*h[0],beds[k][0],.30+D()*.35,.022+strata*.014,.037+strata*.067,-.14,'chip');}}
 ...
 const fiberAudit=[],rejectedFibers=[],huskAudit=[];
 if(family==='adobe'){
  const F=rng(fs);
  ...
  for(let f=0;f<Math.round(384*(c.strawDensity??.6));f++){
 ...
 return {position:new Float32Array(P),normal:new Float32Array(normals),data:new Float32Array(C),kind:new Float32Array(K),index:new Uint32Array(I),stats:{version:'R6.0.0',edgeWear:soft?wear:null,roundRadius:radius,family,seed,shape:c.shape,shapeSeed:ss,damageSeed:ds,colorSeed:cs,fiberSeed:fs,grid:dims,spacing:dx,gridPadding:padding,baseFieldEvaluations:baseValues,localCutterEvaluations:stampValues,normalFieldEvaluations:0,ambiguousCellsResolved:ambiguousCells,removedBodyComponents,removedBodyTriangles,bodyVertices,bodyTriangles:bodyIndexCount/3,bodyIndexCount,vertices:P.length/3,triangles:I.length/3,buildMs:allEnd-begin,stages:{baseMs:baseEnd-begin,cutMs:stampsEnd-baseEnd,meshMs:hullEnd-stampsEnd,attributesAndFibersMs:allEnd-hullEnd},bounds:bodyBounds,fibers:fiberAudit.length,riceHusks:huskAudit.length,huskAudit,mechanicalProfile:hard?'hard-fracture':isStone?'bedded-or-clastic':isPebble?'worn':'moulded',strata:bedded?strata:null,fiberAudit,rejectedFibers,eventCount:eventLog.length,liveAnimation:false},debug:c.debug?{field,dims,org,dx,events:eventLog}:undefined};
}
