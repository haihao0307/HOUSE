/* Brick Mother R4. Independent procedural specimen kernel, 2026-09-06.
...
const defaults={fired:{shape:'sample',damage:.62,relief:.58,grain:.50,color:1,wet:0,rough:.84},stone:{shape:'sample',damage:.55,relief:.74,grain:.55,color:1,wet:0,rough:.90},rubble:{shape:'sample',damage:.62,relief:.68,grain:.60,color:1,wet:0,rough:.90},adobe:{shape:'sample',damage:.46,relief:.55,grain:.50,color:1,wet:0,rough:.97}};
defaults.kiln={...defaults.fired,shape:'long',damage:.47,color:1.15,rough:.83};
defaults.dressed={...defaults.stone,shape:'long',damage:.40,relief:.38,grain:.46,rough:.88};
defaults.pebble={shape:'sample',damage:.06,relief:.22,grain:.24,color:1,wet:0,rough:.80};
function validate(c){if(!c||!defaults[c.family])throw Error('未知材料');for(const k of ['damage','relief'])if(!Number.isFinite(c[k])||c[k]<0||c[k]>1)throw Error('形体参数越界');if(!['sample','long','half','thin','wedge'].includes(c.shape))throw Error('未知形态');if(!Number.isInteger(c.seed)||c.seed<0||c.seed>4294967295)throw Error('主种子需为32位非负整数');if(!Number.isInteger(c.resolution)||c.resolution<40||c.resolution>120)throw Error('采样精度越界');for(const k of ['shapeSeed','damageSeed','colorSeed','fiberSeed'])if(c[k]!==undefined&&(!Number.isInteger(c[k])||c[k]<0||c[k]>4294967295))throw Error('子种子越界');}
function build(c){
 validate(c);const begin=performance.now(),family=c.family,isStone=['stone','rubble','dressed'].includes(family),isBrick=family==='fired'||family==='kiln',isPebble=family==='pebble';
 ...
 const R=rng(ss),D=rng(ds),h=c.shape==='long'?[1.32,.56,.43]:c.shape==='half'?[.74,.80,.44]:c.shape==='thin'?[1.20,.69,.23]:[1.17,.80,.44];
 if(isPebble){h[0]=c.shape==='long'?1.30:c.shape==='half'?.77:1.04;h[1]=c.shape==='thin'?.44:.76;h[2]=c.shape==='thin'?.67:.74;for(let a=0;a<3;a++)h[a]*=.93+R()*.14;}
 ...
 const phase=R()*20,radius=family==='adobe'?.050:.036;
 ...
 for(let z=0;z<nz;z++)for(let y=0;y<ny;y++)for(let x=0;x<nx;x++){
  ...
  if(c.shape==='wedge')sd=Math.max(sd,py+px*.30-h[1]*.78);
 ...
 function cut(center,U,V,W,r,kind){
  const ext=[0,1,2].map(a=>Math.abs(U[a])*r[0]+Math.abs(V[a])*r[1]+Math.abs(W[a])*r[2]+dx);
  ...
  for(let z=lo[2];z<=hi[2];z++)for(let y=lo[1];y<=hi[1];y++)for(let x=lo[0];x<=hi[0];x++){
   ...
   const d=(n-1)*mm+jitter[i]*Math.min(.095,mm*1.12);
   if(-d>field[i]){field[i]=-d;hits++;}
  }
 ...
 const count=Math.round(c.damage*(isPebble?15:isStone?55:92));
 for(let k=0;k<count;k++){
  const axis=isBrick?(k%7<5?2:(k%7===5?1:0)):[2,2,1,0][Math.floor(k/2)%4],sign=isBrick?(k%13===0?-1:1):(k%2?1:-1),oth=[0,1,2].filter(a=>a!==axis);
  const a=(D()*1.92-.96)*h[oth[0]],b=(D()*1.90-.95)*h[oth[1]],large=k%10===0;
  let ru=large?.20+D()*.26:.035+D()*.11,rv=large?.12+D()*.14:ru*(.55+D()*.55),rd=large?.10+D()*.085:.038+D()*.085;
  if(isStone){ru*=1.1;rv*=.58;}
  if(isPebble){ru*=.36;rv*=.36;rd*=.35;}
 ...
 if(c.damage>0&&!isPebble)for(let k=0;k<(isStone?9:5);k++){
 ...
 if(family==='adobe'){
  const F=rng(fs);
  function surface(p,N){let lo=-.58,hi=.19;const value=t=>sample(...p.map((v,k)=>v+N[k]*t));if(value(lo)>=0||value(hi)<=0)return null;for(let j=0;j<22;j++){const mid=(lo+hi)/2;if(value(mid)<0)lo=mid;else hi=mid;}return p.map((v,k)=>v+N[k]*(lo+hi)/2);}
  for(let f=0;f<288;f++){
   ...
   const length=typ===1?.16+F()*.15:.06+F()*.18,width=typ===1?.009+F()*.004:typ===3?.010+F()*.008:typ===2?.004+F()*.006:.0025+F()*.002,
 ...
 return {position:new Float32Array(P),normal:new Float32Array(normals),data:new Float32Array(C),kind:new Float32Array(K),index:new Uint32Array(I),stats:{version:'R4.0.0',family,materialClass:isBrick?'brick':family==='adobe'?'adobe':'stone',seed,shape:c.shape,shapeSeed:ss,damageSeed:ds,colorSeed:cs,fiberSeed:fs,grid:dims,spacing:dx,baseFieldEvaluations:baseValues,localCutterEvaluations:stampValues,normalFieldEvaluations:0,ambiguousCellsResolved:ambiguousCells,removedBodyComponents,removedBodyTriangles,bodyVertices,bodyTriangles:bodyIndexCount/3,bodyIndexCount,vertices:P.length/3,triangles:I.length/3,buildMs:allEnd-begin,stages:{baseMs:baseEnd-begin,cutMs:stampsEnd-baseEnd,meshMs:hullEnd-stampsEnd,attributesAndFibersMs:allEnd-hullEnd},bounds:bodyBounds,fibers:fiberAudit.length,riceHusks:0,fiberAudit,rejectedFibers,eventCount:eventLog.length,liveAnimation:false},debug:c.debug?{field,dims,org,dx,events:eventLog}:undefined};
}
function buildTrio(c){
 const t=performance.now(),parts=[];
 for(let j=0;j<3;j++){
  const p={...c};if(j){p.seed=(c.seed+j*1067)>>>0;for(const [key,tag] of [['shapeSeed','shape'],['damageSeed','damage'],['colorSeed','color'],['fiberSeed','fiber']])p[key]=derive(c[key]??derive(c.seed,tag),'child:'+j);}
  parts.push(build(p));
 }
 const vertices=parts.reduce((n,p)=>n+p.position.length/3,0),triangles=parts.reduce((n,p)=>n+p.index.length/3,0);
 const result={position:new Float32Array(vertices*3),local:new Float32Array(vertices*3),normal:new Float32Array(vertices*3),data:new Float32Array(vertices*4),kind:new Float32Array(vertices),index:new Uint32Array(triangles*3)};
 const bounds={min:[Infinity,Infinity,Infinity],max:[-Infinity,-Infinity,-Infinity]},stride=Math.max(...parts.map(p=>p.stats.bounds.max[0]-p.stats.bounds.min[0]))+.34;let vo=0,io=0;
 for(let j=0;j<3;j++){
  const m=parts[j],shift=(j-1)*stride;result.local.set(m.position,vo*3);result.normal.set(m.normal,vo*3);result.data.set(m.data,vo*4);result.kind.set(m.kind,vo);
  for(let k=0;k<m.position.length;k++){const v=m.position[k]+(k%3===0?shift:0);result.position[vo*3+k]=v;bounds.min[k%3]=Math.min(bounds.min[k%3],v);bounds.max[k%3]=Math.max(bounds.max[k%3],v);}
  for(let k=0;k<m.index.length;k++)result.index[io+k]=m.index[k]+vo;io+=m.index.length;vo+=m.position.length/3;
 }
 result.stats={...parts[0].stats,layout:'trio',parts:parts.map((m,i)=>({seed:m.stats.seed,shiftX:(i-1)*stride,triangles:m.stats.triangles,vertexStart:parts.slice(0,i).reduce((n,q)=>n+q.position.length/3,0),bodyVertices:m.stats.bodyVertices,colorSeed:m.stats.colorSeed})),bounds,vertices,triangles,buildMs:performance.now()-t,
 baseFieldEvaluations:parts.reduce((n,p)=>n+p.stats.baseFieldEvaluations,0),localCutterEvaluations:parts.reduce((n,p)=>n+p.stats.localCutterEvaluations,0),fibers:parts.reduce((n,p)=>n+p.stats.fibers,0),bodyVertices:0,bodyIndexCount:0,fiberAudit:[],debugScope:'per-part geometry already validated separately'};
 return result;
}
return{build,buildTrio,derive,defaults,noise,validate};
}
