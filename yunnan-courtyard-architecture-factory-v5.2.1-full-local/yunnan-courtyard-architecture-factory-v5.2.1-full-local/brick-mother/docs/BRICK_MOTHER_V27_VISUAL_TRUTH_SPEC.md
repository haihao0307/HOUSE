...
离散板块事件大幅退出最终几何，改由前表面的连续低频形成场生成烧结起伏、石材定向层理和土坯压实团块。
事件数组继续承担确定性诊断和局部关联，最终颜色与凹凸减少直线边界和规则板块。

## V2.7.3 参考对比与孔洞校准

直线型事件遮罩退出烧结砖最终画面，孔洞生成偏向审查正面，并压低夸张的轮廓缺口。
烧结砖重新拉开焦黑、灰烬、砖红、氧化与浅色矿物的宽尺度区域，同时降低均匀砂纸状微噪声。
