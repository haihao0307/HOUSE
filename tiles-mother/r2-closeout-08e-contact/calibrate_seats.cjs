...
const report={
  version:'08E',
  method:'Deterministic bilateral rigid-seat solve. Pan y/roll are solved against both rafters; cover y/roll are solved against the two actual adjacent pan identities. Geometry is not deformed by this solver.',
  sampleSeeds,targetMm,
  sourceSeats:S,
  solvedSeats:P,
  panSolve,
  coverSolve,
...
fs.writeFileSync(H+'/SEAT_CALIBRATION.json',JSON.stringify(report,null,2));
