...
function buildRoofLike(kind){
  ...
  const geos={},lod=state.geometryRevision===0?(is48?{nu:16,nv:22}:{nu:10,nv:14}):(is48?{nu:20,nv:30}:{nu:12,nv:20});for(const family of ['pan','cover'])for(let d=0;d<3;d++){geos[`${family}/${d}`]=makeTileGeometry(family,{seed:state.seed+(family==='cover'?2000:0)+d*31,damageClass:d,nu:lod.nu,nv:lod.nv});}
...
function rebuild(){lastRoof=null;$('#contactGate').textContent='落座：切换屋面检查';if(state.scene==='trio')buildTrio();else if(state.scene==='forty8')buildRoofLike('forty8');else if(state.scene==='roof')buildRoofLike('roof');else buildUV();syncUI();runGlobalQA();studyUI();}
