"""Read back immutable public repository HTML through a content-type proxy."""
...
expected=hashlib.sha256((root/'START_HERE.html').read_bytes()).hexdigest()
url=f'https://rawcdn.githack.com/haihao0307/HOUSE/{commit}/tiles-mother/v099/START_HERE.html'
r={'version':'0.9.9','commit':commit,'url':url,'expectedSHA256':expected,'ownPagesDeployed':False,'thirdPartyPreview':True,'visualApproved':False,'productionApproved':False,'errors':[],'consoleErrors':[],'cases':[],'checks':[]}
def save(): (out/'REPORT.json').write_text(json.dumps(r,ensure_ascii=False,indent=2)+'\n')
try:
 start=time.monotonic()
 with urllib.request.urlopen(urllib.request.Request(url,headers={'User-Agent':'TilesMother-Preview-Verification/0.9.9'}),timeout=60) as response:
 ...
 with sync_playwright() as p:
  ...
  page=b.new_page(viewport={'width':1500,'height':950},device_scale_factor=1);page.set_default_timeout(180000)
  ...
  page.wait_for_function("window.TilesMotherV099&&document.body.dataset.ready==='true'")
  ...
  page.click('[data-study="original"]');page.wait_for_timeout(700);assert page.evaluate('TilesMotherV099.state.geometryRevision')==0
  page.click('[data-study="surface"]');page.wait_for_timeout(700);assert page.evaluate('TilesMotherV099.state.geometryRevision')==1
  ...
  page.set_viewport_size({'width':390,'height':844});page.evaluate('TilesMotherV099.setView({scene:"trio",focusSingle:false})');page.wait_for_timeout(400)
