...
const state={showContacts:false,timberOnly:false,scene:'trio',trioFamily:'pan',uvFamily:'both',year:25,care:'maintained',light:'neutral',mode:'material',seed:314159,autoRotate:false,selectedFace:'all',cameraSide:'iso'};
const canvas=$('#stage');
...
function tileMesh(kind,variant,age,{damageClass=0,mode=state.mode}={}){const g=makeTileGeometry(kind,{seed:state.seed+variant*101,damageClass});const mat=mode==='uv'?UV_MATERIALS[kind]:clayMaterial(kind,variant,age,state.light==='rain'?1:0);const m=new THREE.Mesh(g,mat);m.castShadow=m.receiveShadow=true;m.userData={kind,variant,uv:uvGate(g)};if(mode!=='uv')m.material.color.copy(legacyTileTint(variant,age));return m;}
function buildTrio(){clearStage();const fam=state.trioFamily,kinds=fam==='mix'?['pan','cover','pan']:[fam,fam,fam],xs=[-.42,0,.42];kinds.forEach((kind,i)=>{const age=[8,38,74][i],m=tileMesh(kind,i,age,{damageClass:i===2?1:0});m.position.set(xs[i],kind==='pan'?.03:.01,0);m.rotation.set(-.11,(i-1)*.05,(i-1)*.028);m.scale.setScalar(1.58);stageRoot.add(m);caption(`${kind==='pan'?'板瓦':'筒瓦'} 0${i+1}`,new THREE.Vector3(xs[i],-.31,.08),.40);});$('#sceneStats').innerHTML=`<b>三片独立瓦母体</b><span>测绘约值：板瓦 23.8 cm，檐口 24.2 cm，脊口 22.1 cm，弧深约 5.0 cm，厚约 1.2 cm</span><span>筒瓦约 22.2 cm，檐口 11.5 cm，脊口 9.0 cm，弧高约 3.7 cm，厚约 1.0 cm</span><span>外表面、内表面、左右侧边与两端均为独立实体面</span>`;fitCamera('trio',state.cameraSide);}
...
function rebuild(){lastRoof=null;$('#contactGate').textContent='落座：切换屋面检查';if(state.scene==='trio')buildTrio();else if(state.scene==='forty8')buildRoofLike('forty8');else if(state.scene==='roof')buildRoofLike('roof');else buildUV();syncUI();runGlobalQA();}
...
function queued(action){const overlay=$('#runtimeOverlay');$('#runtimeTitle').textContent='正在计算实际搭接与承托';$('#runtimeMessage').textContent='首次打开860片屋面需要几秒至十余秒。已计算的年份会缓存，灯光与回看继续沿用同一份几何。';overlay.hidden=false;requestAnimationFrame(()=>setTimeout(()=>{try{action();}catch(e){window.__tilesShowRuntimeError(e.message);return;}overlay.hidden=true;},20));}
$$('[data-scene]').forEach(b=>b.onclick=()=>{state.scene=b.dataset.scene;state.cameraSide='iso';queued(rebuild);});
