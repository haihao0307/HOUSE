...
const functions=`
 ...
 vec3 tmAlbedo(){vec2 p=vTileUV*2.-1.;p+=.06*vec2(tmFbm(p*4.,tileSeeds.z),tmFbm(p*4.+31.,tileSeeds.z))-.03;
  ...
  float stain=smoothstep(.51,.67,tmFbm(p*3.7+vec2(7.,-4.),tileSeeds.x+53.));
  float pale=smoothstep(.47,.71,tmFbm(p*8.3,tileSeeds.x+97.));
  vec3 c=mix(vec3(.48,.50,.515),vec3(.54,.385,.245),clamp(stain*.66+tileControls.z*.2,0.,.88));
  c+=vec3((macro-.5)*.28*tileControls.x+(meso-.5)*.20*tileControls.y+pale*.07);
  c=mix(c,c*vec3(1.06,.91,.77),vTileFlake*.20);c*=1.-vTileCavity*.11;
  c+=(grain-.5)*.105*tileControls.w;c*=1.-tmWet()*.25;c*=1.-clamp(tileState.y/70.,0.,1.)*.055;
...
function material(d){const uniforms={tileSeeds:{value:new T.Vector4()},tileControls:{value:new T.Vector4(1,1,0,1)},tileState:{value:new T.Vector4(0,0,.84,.35)},tileChannel:{value:0},tileDamage:{value:0}};
 ...
 if(tileChannel!=1.){float bh=(tmGrain()-.5)*.00020*tileState.w+(tmMeso()-.5)*.000055*tileState.w;vec3 qx=dFdx(-vViewPosition),qy=dFdy(-vViewPosition),rx=cross(qy,normal),ry=cross(normal,qx);float det=dot(qx,rx);if(abs(det)>1.e-14)normal=normalize(abs(det)*normal-sign(det)*(dFdx(bh)*rx+dFdy(bh)*ry));}
 `);
...
class Studio{
 ...
 const background=project.channel==='final'&&mode==='studio_beauty'?0xd4d2cc:0xe0e1df;this.scene.background=new T.Color(background);this.ground.material.color.setHex(background);this.ground.visible=project.channel==='final'&&view.pitch>0;this.hemi.intensity=mode==='studio_beauty'?.85:mode==='diagnostic'?1.15:1.7;
 const positions={key:[-1.3,1.7,1.0],fill:[1.4,1.0,.8],rim:[.3,1.3,-1.8]},angle=(mode==='studio_beauty'?stage.rotation:0)*Math.PI/180;
