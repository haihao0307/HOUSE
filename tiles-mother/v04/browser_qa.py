...
report={'version':'0.4.0','tests':[],'pageErrors':[],'consoleErrors':[],'failedRequests':[],'screenshots':[],'visualApproved':False,'productionApproved':False,'allPassed':False,'referenceRegression':'not_run'}
...
try:
 with sync_playwright() as pw:
  ...
  for family in ['pan','cover']:
   ...
   q=pg.evaluate('TilesMother.runQA()');check(family+' builtin geometry checks',q['allPassed'],q['tests'])
   ...
   box=pg.locator('#gl').bounding_box();pg.mouse.move(box['x']+box['width']*.55,box['y']+box['height']*.6);pg.mouse.wheel(0,-180);pg.wait_for_timeout(250);shot(pg,family+'-relief-closeup');pg.locator('#resetView').click();pg.wait_for_timeout(150)
  ...
  # Missing fixtures are a failure in full verification; never silently skip.
  ...
  if not a.quick:
   ...
   with zipfile.ZipFile(zip_path) as z:
    check('collaboration CRC roundtrip',z.testzip() is None);record=json.loads(z.read('workspace.json'));check('V0.4 history included in existing collaboration export',record['project']['study']['physicalTimeSeconds']==50*86400)
    check('original source bytes preserved in export',len(record['referenceFiles'])==2 and all(z.read(r['archivePath'])==(fixture/r['filename']).read_bytes() for r in record['referenceFiles']))
   ...
   zip_path.unlink();report['referenceRegression']='passed_actual_file_roundtrip'
  else:report['referenceRegression']='skipped_quick_mode'
  ...
  if not a.quick:check('reference files persist after reload',len(pg.evaluate('TilesMother.getRefs()'))==2)
