...
function clayMaterial(kind,variant,age,wet=0){
  const ageBand=Math.round(age/5)*5,key=[state.scene,state.seed,kind,variant,ageBand,wet].join('/');if(materialCache.has(key))return materialCache.get(key);
...
function applyTimberOnly(){renderer.shadowMap.needsUpdate=true;stageRoot.traverse(o=>{if(o.userData.kind==='tile-batch')o.visible=!state.timberOnly;if(o.userData.kind==='contact-markers')o.visible=!!state.showContacts;});perfRequest();}
